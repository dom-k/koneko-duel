...
DatabaseAdapter::DatabaseAdapter(const char* db_file) {
...
int DatabaseAdapter::callback(void* data, int argc, char** argv, char** az_col_name) {
...
void DatabaseAdapter::exec(const char* sql_query) {
    if (sqlite3_exec(db, sql_query, this->callback, (void*) data, &this->err_msg) == SQLITE_OK) {
